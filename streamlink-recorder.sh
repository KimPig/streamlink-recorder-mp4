...
while [ true ]; do
	...
	streamlink --plugin-dirs "/usr/local/lib/python3.12/site-packages/streamlink/plugins" $streamOptions $streamLink $streamQuality -O | ffmpeg -fflags +discardcorrupt -i pipe:0 -c:v copy -c:a copy "/home/download/${streamName} - ${streamDate} - ${streamTitle}.mp4"
	sleep 60s
done
