...
while [ true ]; do
	...
	streamlink --ffmpeg-copyts $streamOptions $streamLink $streamQuality -O | ffmpeg -fflags +discardcorrupt -i pipe:0 -c:v copy -c:a copy "/home/download/${streamName} - ${streamDate} - ${streamTitle}.mp4"
	sleep 60s
done
